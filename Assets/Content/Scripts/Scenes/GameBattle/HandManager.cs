...
public class HandManager : MonoBehaviour
{
    ...
    public void HideHand()
    {
        ...
        {
            if (Misc.IdManager.GetCatById(catsInHand[i]) != null)
            {
                Misc.IdManager.GetCatById(catsInHand[i]).graphicsParent.SetActive(false);
    ...
    public void ShowHand()
    {
        for (int i = 0; i < catsInHand.Length; i++)
        {
            if (Misc.IdManager.GetCatById(catsInHand[i]) != null)
            {
                Misc.IdManager.GetCatById(catsInHand[i]).graphicsParent.SetActive(true);
                heartCanvas[i].enabled = true;
                heartText[i].text = Misc.IdManager.GetCatById(catsInHand[i]).health.ToString();
                heartFill[i].fillAmount = Misc.IdManager.GetCatById(catsInHand[i]).health / Misc.IdManager.GetCatById(catsInHand[i]).maxHealth;
            }
            else
            {
                heartCanvas[i].enabled = false;
    ...
    {
        newCatId = Misc.IdManager.GetCatById(_catId).PutInHand();
    }
    ...
    public void ArrangeHand()
    {
        ...
        {
            if (Misc.IdManager.GetCatById(newCatsInHand[i]) != null)
            {
                Misc.IdManager.GetCatById(newCatsInHand[i]).PutInHand();
            }
        }
    }
    ...
    public void HighlightCat(Cat highlightedCat)
    {
        ...
        for (int i = 0;i < catsInHand.Length;i++)
        {
            if (catsInHand[i] != highlightedCat.id && catsInHand[i] != null)
            {
                ...
                heartCanvas[i].enabled = false;
                Misc.IdManager.GetCatById(catsInHand[i]).transform.position = handPoints[i].position;
    ...
    public void DiscardHand()
    {
        for (int i = 0; i < catsInHand.Length; i++)
        {
            if (catsInHand[i] != null)
            {
                Misc.IdManager.GetCatById(catsInHand[i]).Withdraw();
